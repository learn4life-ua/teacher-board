...
- performance-ліміти графіка для екстремальних діапазонів і великого resize;
...
- zoom 50–200% зі стандартним кроком 25%;
- client→scene координати та drag delta на 50/75/100/125/150/175/200%;
- спільний логічний простір 1600×900 для canvas/object/instrument layers;
- resize повернутих об'єктів та інструментів у їхніх локальних осях;
...
- clear → undo → redo для strokes/objects/instruments;
- ізоляцію history між сторінками та ліміт 50 undo-snapshot;
- редагування text/graph через panel-contract без системного prompt.
...
- [x] Static history-check підтверджує clear → undo → redo для strokes/objects/instruments.

## 2. Малювання

- [x] Математика pointer→scene для 75%, 100%, 125%, 150% перевіряється автоматично.
...
- [x] Text/graph мають touch-friendly кнопку ✎, яка відкриває panel editing; legacy text prompt прибрано.
...
- [ ] Resize зображення фізично перевірити після rotation; пропорції мають зберігатися.
...
- [x] Екстремальний діапазон із малим кроком не створює необмежену кількість grid/label/sample елементів.
...
- [x] Scene, canvas, objects та instruments використовують один логічний простір 1600×900.
- [x] Zoom presets: 50%, 75%, 100%, 125%, 150%, 175%, 200%.
- [x] Старі нестандартні zoom-значення нормалізуються до найближчого кроку 25%.
- [x] client→scene та drag delta математично перевіряються на всіх zoom presets.
- [x] Object/instrument resize після rotation перевіряється в локальних осях.
- [ ] Фізично підтвердити, що фон, canvas, об'єкти й інструменти візуально не роз'їжджаються при zoom.
...
### Desktop
- [ ] Ручний сценарій: ручка → гумка → фігура → graph edit → інструмент → зображення → лазер → PNG.

### Tablet
- [ ] Drag/resize/rotate фігур та інструментів.
- [ ] Кнопка ✎ для text/graph → drawer → edit/update.
...
- [ ] Drag/resize повернутого об'єкта пальцем.
- [ ] Лазер.
