...
export class ObjectManager {
  ...
  addGraph(expression = 'x') {
    pushHistory(this.state);
    const obj = createGraphObject(this.state, expression);
    this.objects.push(obj);
    this.state.selection = obj.id;
    this.changed();
    return obj;
  ...
  addSegments(segments, options = {}) {
    ...
    const created = segments.map(({ a, b }) => {
      ...
      const obj = this.createShape('segment', {
        ...
        y: (a.y + b.y) / 2 - 10,
        w: length, h: 20
      }, { minW: 8, minH: 20, rotation: angle, color: options.color, lineWidth: options.lineWidth });
      this.objects.push(obj);
      return obj;
    });
  ...
  pointerDownObject(e, obj) {
    ...
    e.preventDefault(); e.stopPropagation(); this.select(obj.id);
    const handle = e.target.closest('[data-handle]')?.dataset.handle;
    pushHistory(this.state);
    this.drag = { mode:handle || 'move', id:obj.id, startX:e.clientX, startY:e.clientY, x:obj.x, y:obj.y, w:obj.w, h:obj.h, rotation:obj.rotation || 0, center:{x:obj.x+obj.w/2,y:obj.y+obj.h/2} };
  }
