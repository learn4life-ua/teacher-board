...
export class ObjectManager {
  ...
  select(id){this.state.selection=id||null;this.render();}
  deleteSelected(){const id=this.state.selection;if(!id)return;const i=this.objects.findIndex(o=>o.id===id);if(i<0)return;pushHistory(this.state);this.objects.splice(i,1);this.state.selection=null;this.changed();}
  updateSelected(patch){const obj=this.selected();if(!obj)return;pushHistory(this.state);Object.assign(obj,patch);this.changed();}

  render(){this.layer.innerHTML='';for(const obj of this.objects)this.layer.appendChild(this.createElement(obj));}
  createElement(obj){
    const el=document.createElement('div');
    el.className=`scene-object${obj.kind==='graph'?' graph-object':''}${obj.kind==='text'?' text-object':''}${obj.kind==='image'?' image-object':''}${obj.id===this.state.selection?' selected':''}`;
    ...
    if(obj.id===this.state.selection) el.insertAdjacentHTML('beforeend','<span class="object-handle resize-handle" data-handle="resize" title="Змінити розмір"></span><span class="object-handle rotate-handle" data-handle="rotate" title="Повернути">↻</span><button class="object-delete" type="button" title="Видалити">×</button>');
  ...
  pointerDownObject(e,obj){if(this.state.tool!=='select')return;e.preventDefault();e.stopPropagation();this.select(obj.id);const handle=e.target.closest('[data-handle]')?.dataset.handle;pushHistory(this.state);this.drag={mode:handle||'move',id:obj.id,startX:e.clientX,startY:e.clientY,x:obj.x,y:obj.y,w:obj.w,h:obj.h,rotation:obj.rotation||0,center:{x:obj.x+obj.w/2,y:obj.y+obj.h/2}};}
