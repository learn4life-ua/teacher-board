...
function renderPages(){pagesEl.innerHTML='';state.pages.forEach((page,i)=>{const b=document.createElement('button');b.className=`page-tab${i===state.activePage?' active':''}`;b.textContent=`${i+1}. ${page.name}`;b.addEventListener('click',()=>{state.activePage=i;state.selection=null;resetHistory(state);commit();});b.addEventListener('dblclick',()=>renamePage(i));pagesEl.appendChild(b);});$('#deletePageBtn').disabled=state.pages.length<=1;}
...
function renamePage(index){const page=state.pages[index];if(!page)return;const next=prompt('Назва сторінки:',page.name||`Сторінка ${index+1}`);if(next?.trim()){page.name=next.trim();commit();}}
...
function bindResponsivePanel(){$('#mobilePanelBtn').addEventListener('click',()=>document.body.classList.contains('side-panel-open')?closeSidePanel():openSidePanel());$('#closeSidePanelBtn').addEventListener('click',closeSidePanel);$('#panelScrim').addEventListener('click',closeSidePanel);window.addEventListener('resize',()=>{if(innerWidth>900)closeSidePanel();},{passive:true});}

function bindUi(){$$('.tool[data-tool]').forEach(b=>b.addEventListener('click',()=>setTool(b.dataset.tool)));$$('.instrument-btn').forEach(b=>b.addEventListener('click',()=>{geometryTools.add(b.dataset.instrument);setTool('select');}));$('#shapeBtn').addEventListener('click',e=>{e.stopPropagation();shapeMenu.hidden=!shapeMenu.hidden;});document.addEventListener('click',e=>{if(!e.target.closest('#shapeMenu')&&!e.target.closest('#shapeBtn'))shapeMenu.hidden=true;});$('#colorPicker').addEventListener('input',e=>{state.color=e.target.value;const selected=objectManager.selected();if(selected&&!['image'].includes(selected.kind))objectManager.updateSelected({color:state.color});else persistState();});$('#lineWidth').addEventListener('change',e=>{state.lineWidth=Number(e.target.value);const selected=objectManager.selected();if(selected&&!['graph','text','image'].includes(selected.kind))objectManager.updateSelected({lineWidth:state.lineWidth});else persistState();});$('#zoomInBtn').addEventListener('click',()=>{scene.setZoom(state.zoom+.1);persistState();geometryTools.render();});$('#zoomOutBtn').addEventListener('click',()=>{scene.setZoom(state.zoom-.1);persistState();geometryTools.render();});$('#undoBtn').addEventListener('click',()=>{if(undo(state))commit();});$('#redoBtn').addEventListener('click',()=>{if(redo(state))commit();});$('#duplicatePageBtn').addEventListener('click',duplicatePage);$('#renamePageBtn').addEventListener('click',renameCurrentPage);$('#deletePageBtn').addEventListener('click',deletePage);$('#clearPageBtn').addEventListener('click',clearPage);$('#fullscreenBtn').addEventListener('click',toggleFullscreen);$('#deleteBtn').addEventListener('click',()=>objectManager.deleteSelected());$('#addPageBtn').addEventListener('click',addPage);$$('.background-btn').forEach(b=>b.addEventListener('click',()=>{pushHistory(state);activePage(state).background=b.dataset.bg;commit();}));sceneEl.addEventListener('pointerdown',e=>{if(state.tool==='select'&&!e.target.closest('.scene-object,.geometry-tool'))objectManager.select(null);});window.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='z'){e.preventDefault();if(e.shiftKey?redo(state):undo(state))commit();}if((e.key==='Delete'||e.key==='Backspace')&&state.selection&&!e.target.closest('input,textarea')){e.preventDefault();objectManager.deleteSelected();}if(e.key==='Escape'){setTool('select');closeSidePanel();}});}

buildShapeMenu();bindShapeDrawing();bindGraphPanel();bindTextPanel();bindImages();bindResponsivePanel();bindUi();setTool(state.tool.startsWith('shape:')?'select':state.tool);renderAll();