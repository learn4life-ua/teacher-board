...
export class GeometryTools{
  ...
  element(item){const el=document.createElement('div');el.className=`geometry-tool geometry-${item.type}`;el.dataset.id=item.id;Object.assign(el.style,{left:`${item.x}px`,top:`${item.y}px`,width:`${item.w}px`,height:`${item.h}px`,transform:`rotate(${item.rotation||0}deg)`});el.innerHTML=`${this.svg(item)}${this.actionMarkup(item)}<button class="geometry-close" type="button" title="Закрити">×</button><span class="geometry-rotate" data-handle="rotate" title="Повернути">↻</span><span class="geometry-resize" data-handle="resize" title="Змінити розмір"></span>${item.type==='protractor'?this.angleHandleMarkup(item):''}${item.type==='compass'?this.compassControls(item):''}`;
    ...
    el.querySelectorAll('.compass-mode').forEach(b=>{b.addEventListener('pointerdown',e=>e.stopPropagation());b.addEventListener('click',e=>{e.stopPropagation();const next=b.dataset.mode;if(item.mode===next)return;pushHistory(this.state);item.mode=next;this.render();this.onChange?.();});});return el;}
  ...
  pointerDown(e,item){if(this.drag)return;e.preventDefault();e.stopPropagation();const handle=e.target.closest('[data-handle]')?.dataset.handle||'move';this.drag={id:item.id,mode:handle,pointerId:e.pointerId??null,startX:e.clientX,startY:e.clientY,x:item.x,y:item.y,w:item.w,h:item.h,rotation:item.rotation||0,radius:item.radius||92,centerX:item.x+item.w/2,centerY:item.y+item.h/2,historyPushed:false};}
  ...
  pointerMove(e){if(!this.drag||!this.matchesPointer(e))return;const item=this.items.find(x=>x.id===this.drag.id);if(!item)return;const screenDx=e.clientX-this.drag.startX,screenDy=e.clientY-this.drag.startY;if(!this.ensureDragHistory(screenDx,screenDy))return;const z=this.state.zoom||1,delta=sceneDeltaFromClient(screenDx,screenDy,z),dx=delta.x,dy=delta.y;if(this.drag.mode==='move'){item.x=clamp(this.drag.x+dx,-item.w+40,1560);item.y=clamp(this.drag.y+dy,-item.h+40,860);}else if(this.drag.mode==='resize'){const local=sceneDeltaToLocalAxes(dx,dy,this.drag.rotation),rdx=local.x,rdy=local.y;item.w=clamp(this.drag.w+rdx,item.type==='ruler'?260:180,MAX_INSTRUMENT_W);item.h=item.type==='ruler'?clamp(this.drag.h+rdy*.2,72,240):clamp(this.drag.h+rdy,140,MAX_INSTRUMENT_H);if(item.type==='compass')item.radius=clamp(Number(item.radius)||92,30,Math.max(45,Math.min(item.w,item.h)*.45));}else if(this.drag.mode==='rotate'){const r=this.layer.getBoundingClientRect(),cx=r.left+this.drag.centerX*z,cy=r.top+this.drag.centerY*z;item.rotation=Math.atan2(e.clientY-cy,e.clientX-cx)*180/Math.PI+90;}else if(this.drag.mode==='angle'&&item.type==='protractor'){const r=this.layer.getBoundingClientRect(),cx=r.left+(item.x+item.w/2)*z,cy=r.top+(item.y+item.h*.909)*z;item.angle=protractorAngleFromPointer({pointerX:e.clientX,pointerY:e.clientY,pivotX:cx,pivotY:cy,rotation:item.rotation||0});}else if(this.drag.mode==='radius'&&item.type==='compass'){item.radius=compassRadiusFromDrag({startRadius:this.drag.radius,dx,dy,rotation:item.rotation||0,min:30,max:Math.max(45,Math.min(item.w,item.h)*.45)});}this.render();}
  pointerUp(e){if(!this.drag||!this.matchesPointer(e))return;const changed=this.drag.historyPushed;this.drag=null;if(changed)this.onChange?.();}
