...
export function protractorAngleFromPointer({pointerX,pointerY,pivotX,pivotY,rotation=0}){
  ...
  // A semicircular school protractor has only the upper 0..180° scale.
...
export class GeometryTools{
  ...
  buildArc(item){const center=this.localToScene(item,item.w*.28,item.h*.90),radius=Math.max(30,Number(item.radius)||92);this.objectManager.addArc(center,radius,item.arcStart||0,item.arcEnd||180,{color:this.state.color,lineWidth:this.state.lineWidth});}
  ...
  compassSvg(item){const r=Math.round(item.radius||92),max=Math.max(45,Math.min(item.w,item.h)*.45),p=clamp(r/max,.25,1),rightX=28+(78-28)*p,midX=(28+rightX)/2;return `<svg viewBox="0 0 100 120"><circle cx="50" cy="14" r="8"/><line x1="47" y1="22" x2="28" y2="108"/><line x1="53" y1="22" x2="${rightX.toFixed(2)}" y2="108"/><circle cx="28" cy="108" r="3"/><circle cx="${rightX.toFixed(2)}" cy="108" r="3"/><path d="M28 108 A55 55 0 0 1 ${rightX.toFixed(2)} 108" class="guide"/><line x1="50" y1="38" x2="${midX.toFixed(2)}" y2="67"/><circle cx="${midX.toFixed(2)}" cy="67" r="3"/><text x="50" y="82" text-anchor="middle" class="compass-readout">r = ${r}</text></svg>`;}
}
