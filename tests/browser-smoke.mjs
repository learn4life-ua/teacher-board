...
async function runLegacyMigrationCase(){
  ...
  try{
    ...
    const migrated=await page.evaluate(()=>({v1:localStorage.getItem('teacherboard.v1'),flag:localStorage.getItem('teacherboard.v2.migratedFromV1'),v2:JSON.parse(localStorage.getItem('teacherboard.v2')||'null')}));
    ...
    assert.equal(raster.x,0);assert.equal(raster.y,0);assert.equal(raster.w,1600);assert.equal(raster.h,900);
    assert.ok(text,'migration: legacy text was not converted to a text object');
    assert.equal(text.x,123);assert.equal(text.y,234);assert.equal(text.color,'#123456');
    ...
    await page.reload({waitUntil:'networkidle'});await page.waitForSelector('#scene');
...
async function runLegacyRollbackCase(){
  ...
  const browser=await chromium.launch({headless:true});
  try{
    const context=await browser.newContext({viewport:{width:1100,height:800}});const page=await context.newPage();page.setDefaultTimeout(8000);
    await page.addInitScript(data=>{
      localStorage.removeItem('teacherboard.v2');localStorage.removeItem('teacherboard.v2.migratedFromV1');localStorage.setItem('teacherboard.v1',JSON.stringify(data));
      const original=Storage.prototype.setItem;Storage.prototype.setItem=function(key,value){if(key==='teacherboard.v2')throw new DOMException('Simulated storage quota failure','QuotaExceededError');return original.call(this,key,value);};
    },legacyFixture);
    await page.goto(baseURL,{waitUntil:'networkidle'});await page.waitForSelector('#scene');
    const state=await page.evaluate(()=>({v1:localStorage.getItem('teacherboard.v1'),v2:localStorage.getItem('teacherboard.v2'),flag:localStorage.getItem('teacherboard.v2.migratedFromV1')}));
    assert.ok(state.v1,'migration rollback: original v1 was not restored after v2 write failure');
    assert.equal(JSON.parse(state.v1).pages?.[0]?.name,'Старий урок');assert.equal(state.v2,null);assert.equal(state.flag,null);
    console.log('[migration-rollback] v1 restored after failed v2 write');
  }finally{await browser.close();}
}

async function desktopDrawingExtras(page){
  const box=await page.locator('#drawingCanvas').boundingBox();assert.ok(box,'desktop: drawing canvas box missing');
  const from={x:Math.min(520,box.width*.46),y:Math.min(390,box.height*.46)},to={x:from.x+120,y:from.y};
  const sampleX=Math.round((from.x+to.x)/2),sampleY=Math.round(from.y);
  await page.click('.tool[data-tool="pen"]');await page.mouse.move(box.x+from.x,box.y+from.y);await page.mouse.down();await page.mouse.move(box.x+to.x,box.y+to.y,{steps:8});await page.mouse.up();
  const alphaBefore=await page.locator('#drawingCanvas').evaluate((canvas,{x,y})=>canvas.getContext('2d').getImageData(x,y,1,1).data[3],{x:sampleX,y:sampleY});assert.ok(alphaBefore>0,'desktop: pen stroke did not paint canvas');
  await page.click('.tool[data-tool="eraser"]');await page.mouse.move(box.x+from.x,box.y+from.y);await page.mouse.down();await page.mouse.move(box.x+to.x,box.y+to.y,{steps:8});await page.mouse.up();
  const alphaAfter=await page.locator('#drawingCanvas').evaluate((canvas,{x,y})=>canvas.getContext('2d').getImageData(x,y,1,1).data[3],{x:sampleX,y:sampleY});assert.ok(alphaAfter<alphaBefore,'desktop: eraser did not remove drawing canvas alpha');
  await page.click('.tool[data-tool="select"]');console.log('[desktop] transparent eraser ok');
}

async function desktopExtras(page){
  const beforeCurtain=await page.locator('.scene-object').count();await drawShape(page,'curtain',{x:55,y:80},{x:180,y:165});
...
async function geometryExtras(page){
  const beforeRuler=await page.locator('.scene-object').count();await page.locator('.geometry-ruler .geometry-action').click();assert.equal(await page.locator('.scene-object').count(),beforeRuler+1,'desktop: ruler did not construct segment');
  await page.locator('.instrument-btn[data-instrument="protractor"]').click();assert.equal(await page.locator('.geometry-protractor .angle-readout').count(),1,'desktop: protractor readout missing');
  const beforeAngle=await page.locator('.scene-object').count();await page.locator('.geometry-protractor .geometry-action').click();assert.equal(await page.locator('.scene-object').count(),beforeAngle+2,'desktop: protractor did not construct two rays');
  await page.locator('.instrument-btn[data-instrument="compass"]').click();assert.equal(await page.locator('.geometry-compass .compass-readout').count(),1,'desktop: compass readout missing');
  const beforeCircle=await page.locator('.scene-object').count();await page.locator('.geometry-compass .geometry-action').click();assert.equal(await page.locator('.scene-object').count(),beforeCircle+1,'desktop: compass did not construct circle');
  await page.locator('.geometry-compass .compass-mode[data-mode="arc"]').click();const beforeArc=await page.locator('.scene-object').count();await page.locator('.geometry-compass .geometry-action').click();assert.equal(await page.locator('.scene-object').count(),beforeArc+1,'desktop: compass did not construct arc');
...
async function runCase(name,contextOptions={}){
  console.log(`[${name}] start`);const browser=await chromium.launch({headless:true});
  try{
    const context=await browser.newContext({acceptDownloads:true,...contextOptions});const page=await context.newPage();page.setDefaultTimeout(8000);const errors=[];
    page.on('pageerror',e=>errors.push(String(e)));page.on('console',m=>{if(m.type()==='error')errors.push(m.text());});
    await page.goto(baseURL,{waitUntil:'networkidle'});await page.waitForSelector('#scene');const narrow=(contextOptions.viewport?.width||9999)<=900||contextOptions.isMobile;
    assert.equal(await page.locator('#shapeMenu').isHidden(),true,`${name}: shape menu must start closed`);assert.equal(await page.locator('.tool.active[data-tool="select"]').count(),1,`${name}: select must be active on startup`);
    if(name==='desktop')await desktopDrawingExtras(page);
    await drawShape(page,'rect',{x:220,y:180},{x:430,y:320});assert.ok(await page.locator('.scene-object').count()>=1,`${name}: shape object was not created`);console.log(`[${name}] shape ok`);if(name==='desktop')await desktopExtras(page);
    ...
    if(name==='desktop'){
      ...
      const saved=await page.evaluate(()=>JSON.parse(localStorage.getItem('teacherboard.v2')||'null'));assert.ok(saved?.pages?.length>=1);assert.ok(saved.pages.some(p=>Array.isArray(p.objects)&&p.objects.length>0));assert.ok(saved.pages.some(p=>Array.isArray(p.strokes)&&p.strokes.some(s=>s.tool==='eraser')));console.log('[desktop] autosave ok');
      await clearUndoExtras(page);await zoomExtras(page);
    }
    const beforePages=await page.locator('.page-tab').count();await pageActions(page,name,beforePages);
    if(name==='desktop'){const pending=page.waitForEvent('download',{timeout:15000});await page.click('#savePngBtn');const download=await pending;assert.match(download.suggestedFilename(),/\.png$/i);console.log('[desktop] png ok');}
    assert.deepEqual(errors,[],`${name}: browser errors: ${errors.join(' | ')}`);console.log(`[${name}] ok`);
  }finally{await browser.close();}
}
